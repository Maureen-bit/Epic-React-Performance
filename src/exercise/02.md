...
## Comparing the production and developer modes' performance
In the developer mode, the environment has to run different configurations to improve
and help the developer experience. Therefore, the speed of running an application is lower than in production mode. We notice that when we run the project in production mode, we don't have access to develeper tools such as, timings in the performance tab or the react tools from the extension. When we're building our app, It minifies the code. It sets React in production mode, which will remove all of those development helper things that it does. That's why the production app is faster.
